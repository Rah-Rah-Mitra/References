...
cols = int(win.get_width()/w) # 60
rows = int(win.get_height()/w) # 36
...
class Cell:
    ...
    def show(self, win):
        x = self.i * w  #i*w = x-pos of rect       #w = move increments / cube size
        y = self.j * w  #j*w = y-pos of rect
        if self.visited:
            pygame.draw.rect(win, (190, 90, 90), (self.i*w, self.j*w, w, w))
        if self.walls[0]:
            pygame.draw.line(win, (0, 0, 0), (x,y), (x+w,y)) # Line on top of Cube
        if self.walls[1]:    
            pygame.draw.line(win, (0, 0, 0), (x+w,y), (x+w,y+w)) # Line on right of Cube
        if self.walls[2]:    
            pygame.draw.line(win, (0, 0, 0), (x+w,y+w), (x,y+w)) # Line on bottom of Cube
        if self.walls[3]:
            pygame.draw.line(win, (0, 0, 0), (x,y+w), (x,y)) # Line on left of Cube
